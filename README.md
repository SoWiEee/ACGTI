...
- **纯前端架构**：无后端、无注册，测试结果全部在本地浏览器完成计算。站点含第三方广告服务（Google AdSense），详见下方隐私说明。
...
- 本站页面包含由 **Google AdSense** 提供的第三方广告服务，广告脚本可能使用 Cookie 进行个性化广告投放。这属于第三方服务行为，不属于本项目的数据收集范畴。如需了解广告隐私政策，请参阅 [Google 隐私权政策](https://policies.google.com/privacy) 与 [广告设置](https://adssettings.google.com/)。
